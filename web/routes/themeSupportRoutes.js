...
const validBlockType =
	process.env.NODE_ENV === "production"
		? "shopify://apps/justupsell-checkbox-upsell/blocks/justUpsell/"
		: "shopify://apps/justupsell-dev/blocks/justUpsell/"
...
export default function applyThemeSupportRoutes(app) {
			...
			const sectionsWithAppBlock = (
				await Promise.all(
					templateMainSections.map(async (file, index) => {
						...
						const match = asset[0].value.match(/\{%\s+schema\s+%\}([\s\S]*?)\{%\s+endschema\s+%\}/m)
		...
		try {
			...
			if (themes?.length > 0) {
				...
				if (!settingsDataAssets[0]?.value) {
					logger.warn("/api/checkIfAppBlockIsEnabled no value field found in settings_data.json", {
						settingsDataAsset: settingsDataAssets[0],
					})
					return res.status(500).send({ response: false })
				}
	...
	app.get("/api/isScriptTagPresent", async (req, res) => {
		try {
			...
			scripts.map((script) => {
				if (script.src.includes(`${process.env.HOST}/carouselScript.js`)) {
	...
	app.post("/api/createScriptTag", async (req, res) => {
		try {
			...
			scriptTag.src = `${process.env.HOST}/carouselScript.js?loaddeps`
