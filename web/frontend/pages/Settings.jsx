...
import { useUpdatePreferredCarouselPositioning } from "../hooks/carouselHooks.js"
import { useFirestoreDataUpdate } from "../hooks/useFirestoreDataUpdate.js"

const Settings = () => {
	const updateFirestoreData = useFirestoreDataUpdate()
	const updatePreferredCarouselPositioning = useUpdatePreferredCarouselPositioning()
	...
	const save = async () => {
		setSaving(true)
		await updateFirestoreData({ preferredCarouselPositioning: state.preferredCarouselPositioning })
		const res = await updatePreferredCarouselPositioning(state.preferredCarouselPositioning)
		setSaving(false)
			...
			<div style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", paddingTop: "2rem" }}>
				<div className="neu-background neu-shadow neu-border-radius-1" style={{ width: "700px", padding: "2rem" }}>
					<div style={{ marginBottom: "1rem", paddingBottom: "1rem", borderBottom: "1px solid #ccc" }}>
						<p className="neu-text neu-text-600 font-satoshi" style={{ fontSize: "1.3rem" }}>
							Settings
						</p>
					</div>
