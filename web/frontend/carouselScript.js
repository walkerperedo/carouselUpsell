...
async function getCarousel(productId, store) {
	return await fetch(`${apiHost}/api/carousels/getCarousel/${productId}?store=${store}`, { method: "PUT" })
		.then((res) => res.json())
		.then((json) => {
			if (json.error) {
...
async function onAddToCartClicked(e) {
	...
	await addToCart(
		variantIdsToAddToCart.map((variantId) => {
			const upsellId = fetchedCarousel.find((x) => x.upsellVariantId === variantId)?.id || ""
			return {
				quantity: 1,
...
function showModal(upsell, upsellDomElement) {
	...
	const imageMarkup = upsell.image
...
async function addToCartModalProduct(upsell, button, modal) {
	...
	addToCart([{ quantity: 1, id: parseInt(upsell.upsellVariantId), properties: { _justUpsell: upsell.id } }])
		.then(() => {
...
async function initCarousel() {
	...
	const carouselAppBlockContainer = document.querySelector("#carouselAppBlock-container")
	const productIdFromAppBlock = carouselAppBlockContainer?.getAttribute("productid")
	...
	const productId =
		productIdFromAppBlock || productIdFromMetaProductId || productIdFromMetaProductGid || productIdFromMetaPageResourceId || ""
	window.carouselProductId = productId
	...
	if (addToCartButton && productId) {
		...
		const carousel = await getCarousel(productId, store)
